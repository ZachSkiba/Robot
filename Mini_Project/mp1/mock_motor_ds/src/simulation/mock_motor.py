...
class MockMotor:
    def __init__(self, max_velocity=MAX_VELOCITY, max_accel=MAX_ACCEL, max_safe_vel=None):
        """
        Universal MockMotor.
        Safe to import into other scripts without side effects.
        """
        # Legacy support
        if max_safe_vel is not None:
            self.max_velocity = max_safe_vel
        else:
    ...
    def update(self, target_pos, target_velocity, dt):
        """
        ...
        # Limit desired velocity by remaining distance (P-Control logic)
        desired_vel = np.sign(dist_to_go) * min(abs(target_velocity), abs(dist_to_go/dt))

        # Limit acceleration (Inertia logic)
        vel_diff = desired_vel - self.current_velocity
        
        # Use instance-specific max_accel
...
if __name__ == "__main__":
    ...
    def run_simulation():
        ...
        for speed in SPEEDS:
            ...
            print(f"✅ Data saved to: {filename}, time-to-360°={time_points[len(time_points)-hold_steps-1]:.3f} s")
